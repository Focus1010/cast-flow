  ...
  try {
    ...
    const isFarcasterEnvironment = () => {
      ...
      const isFarcaster = (
        ...
        window !== window.top ||
        // Check for Farcaster mobile app indicators
        userAgent.includes('mobile') && referrer === '' && window !== window.top
      );
    ...
    if (isFarcasterEnvironment() && sdk.init && typeof sdk.init === 'function') {
      try {
        await sdk.init();
